#!/usr/bin/env node
// 요한복음 9장 추가 마이그레이션 — 1~8장은 이미 있으므로 건드리지 않는다.
// 9장은 완전히 새 장이라 upsert만으로 충분하다(기존 행과 안 겹침).
//
// 입력 JSON은 요한복음 1~8장 때와 같은 평평한 배열 형식이다:
//   john_verses_9.json — [{chapter, verse, text, sect?}, ...]
//   john_notes_9.json  — [{chapter, verse_start, verse_end, title, body, grammar_note, refs, src}, ...]
//
// 사용법
//   cd supabase
//   node migrate_john_9.js --dry-run
//   node migrate_john_9.js
...
const VERSES_PATH = path.join(__dirname, 'migration-output', 'john_verses_9.json');
const NOTES_PATH = path.join(__dirname, 'migration-output', 'john_notes_9.json');
...
function buildVerseRows(versesArr) {
  ...
  return versesArr.map((v) => {
    if (v.chapter !== CHAPTER) throw new Error(`verses 파일에 9장이 아닌 장(${v.chapter}장)이 있습니다.`);
    return {
      book_id: 'john',
      translation_id: 'krv',
      chapter: v.chapter,
...
function buildNoteRows(notesArr) {
  return notesArr.map((n) => {
    if (n.chapter !== CHAPTER) throw new Error(`notes 파일에 9장이 아닌 장(${n.chapter}장)이 있습니다.`);
    return {
      book_id: 'john',
      chapter: n.chapter,
...
async function main() {
  ...
  console.log(`  verses: ${verseRows.length}절 (9장)`);
