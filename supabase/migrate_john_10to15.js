#!/usr/bin/env node
// 요한복음 10~15장 추가 마이그레이션 — 1~9장은 이미 있으므로 건드리지 않는다.
// 10~15장은 완전히 새 장이라 upsert만으로 충분하고(기존 행과 안 겹침), notes도
// 델리트 없이 그냥 upsert한다.
//
// 입력 JSON은 요한복음 6~8장 때와 같은 평평한 배열 형식이다:
//   john_verses_10to15.json — [{chapter, verse, text, sect?}, ...]
//   john_notes_10to15.json  — [{chapter, verse_start, verse_end, title, body, grammar_note, refs, src}, ...]
//
// 사용법
//   cd supabase
//   node migrate_john_10to15.js --dry-run
//   node migrate_john_10to15.js
...
const VERSES_PATH = path.join(__dirname, 'migration-output', 'john_verses_10to15.json');
const NOTES_PATH = path.join(__dirname, 'migration-output', 'john_notes_10to15.json');

const CHAPTER_MIN = 10;
const CHAPTER_MAX = 15;
...
function buildVerseRows(versesArr) {
  ...
  return versesArr.map((v) => {
    if (v.chapter < CHAPTER_MIN || v.chapter > CHAPTER_MAX) {
      throw new Error(`verses 파일에 범위 밖 장(${v.chapter}장)이 있습니다 — 10~15장만 허용됩니다.`);
    }
    return {
      book_id: 'john',
      translation_id: 'krv',
...
function buildNoteRows(notesArr) {
  return notesArr.map((n) => {
    if (n.chapter < CHAPTER_MIN || n.chapter > CHAPTER_MAX) {
      throw new Error(`notes 파일에 범위 밖 장(${n.chapter}장)이 있습니다 — 10~15장만 허용됩니다.`);
    }
    return {
      book_id: 'john',
      chapter: n.chapter,
