#!/usr/bin/env node
// CHAPTER_LENGTHS_BY_BOOK vs Supabase KRV 전수 감사
// 조회/검증만. INSERT/UPDATE/DELETE 없음.
'use strict';
...
async function main() {
  ...
  process.stdout.write(`CHAPTER_LENGTHS_BY_BOOK vs Supabase KRV 전수 감사\n`);
  process.stdout.write(`대상: ${books.length}권\n\n`);
  process.stdout.write('책별 조회 중 ');

  for (const bookId of books) {
    const chapters = CLB[bookId];
    ...
    for (const r of allRows) {
      actualByChap[r.chapter] = (actualByChap[r.chapter] || 0) + 1;
    }

    for (const ch of chNums) {
      totalChapters++;
      const expected = chapters[ch];
      const actual = actualByChap[ch] || 0;
      if (expected !== actual) {
        mismatches.push({ book: bookId, chapter: ch, clb: expected, supabase: actual, diff: actual - expected });
      } else {
        matchCount++;
      }
    }
  ...
  console.log('CHAPTER_LENGTHS_BY_BOOK 전수 감사 결과');
  console.log('='.repeat(70));
  console.log(`전체 책:   ${books.length}`);
  console.log(`전체 장:   ${totalChapters}`);
  console.log(`일치 장:   ${matchCount}`);
  console.log(`불일치 장: ${mismatches.length}`);
  const pct = ((mismatches.length / totalChapters) * 100).toFixed(2);
  console.log(`불일치율:  ${pct}%`);
  console.log('');

  if (mismatches.length > 0) {
    console.log('불일치 목록 (book | 장 | CLB값 | Supabase실제 | 차이):');
    console.log('-'.repeat(70));
    for (const m of mismatches) {
      const diff = m.diff > 0 ? `+${m.diff}` : String(m.diff);
      console.log(`${m.book.padEnd(20)} | ${String(m.chapter).padStart(3)}장 | CLB=${String(m.clb).padStart(3)} | Supabase=${String(m.supabase).padStart(3)} | diff=${diff}`);
    }
    console.log('');
    console.log('CHAPTER_LENGTHS_FULL_AUDIT = FAIL');
  } else {
    console.log('CHAPTER_LENGTHS_FULL_AUDIT = PASS');
  }
}
